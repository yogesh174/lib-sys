<script context="module">
	export async function load({ session }) {
		...
		const response = await api.get('issues', session.jwt);
		...
		return {
			error: 'Uh oh! Could not fetch data!'
		};
	}
</script>
...
<script>
	...
	let issued = issues.filter((issue) => issue.status === 'issued');
	let returned = issues.filter((issue) => issue.status === 'returned');
